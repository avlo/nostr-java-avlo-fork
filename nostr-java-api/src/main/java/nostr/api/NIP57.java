...
public class NIP57 extends Nostr {
  ...
  public static GenericEvent createZapEvent(@NonNull Integer amount, @NonNull String lnurl, Relay relay, @NonNull PublicKey recipient, EventTag eventTag, String content) {
    ...
    GenericEvent event = new GenericEventImpl(sender);
    event.setKind(Kind.valueOf(9734));
    event.setTags(tags);
   ...
   * @return
   */
  public static GenericEvent createZapReceiptEvent(@NonNull GenericEventImpl zapEvent, @NonNull String bolt11, String description, @NonNull String preimage, @NonNull PublicKey recipient, @NonNull EventTag eventTag) {
    var tags = new ArrayList<BaseTag>();
    ...
    var sender = Identity.getInstance().getPublicKey();
    GenericEvent event = new GenericEventImpl(sender);
    event.setKind(Kind.valueOf(9735));
    event.setTags(tags);
    event.setContent(Nostr.Json.encode(zapEvent));
    return event;
  }
